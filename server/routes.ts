  ...
  app.get('/download/cleantraffic-package', (req, res) => {
    import('fs').then(fs => {
      import('path').then(path => {
        const packagePath = path.resolve('./CleanTraffic-PHP-Protection-Package-Final-Fixed.tar.gz');
        const fileName = 'CleanTraffic-PHP-Protection-Package-Final-Fixed.tar.gz';
